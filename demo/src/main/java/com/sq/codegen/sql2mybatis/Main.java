...
public class Main {
    ...
    private static void genMybatis() {
        ...
        List<TableFields> result = mysql.QueryTableScheme("xone_data", "t_cooperate_product");
    ...
    private static String SimpleFields = "/**\n* %s\n*/"
        + "\nprivate %s %s;\n";
    private static  void ImpSimple(List<TableFields> result){
        StringBuilder sb = new StringBuilder();
        for (TableFields f : result) {
            String javaType = TypeConvert.getJavaType(f.getType());
            String cameFieldName = StrUtils.toCamelCase(f.getName());
            final String text = String.format(SimpleFields,
                f.getComments(),
                javaType,
                cameFieldName);
            sb.append(text);
    ...
    private static void genSimple() {
        MysqlConnect mysql = new MysqlConnect();
        List<TableFields> result = mysql.QueryTableScheme("xone_data", "t_log");
        ImpSimple(result);
    }
